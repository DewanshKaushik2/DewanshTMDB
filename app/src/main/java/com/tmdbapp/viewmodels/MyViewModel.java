...
public class MyViewModel extends ViewModel {
    ...
    public LiveData<PagedList<MovieModel>> getPopularMovies(Context context) {
        ...
        if (preferencesLimit != null) movieLimit = Integer.parseInt(preferencesLimit);
...
//10,20,
    public void getPopularMoviesOnline() {
        if (NetworkUtils.isNetworkAvailable(this.contextWeakReference.get())) {
            for (int page = 1; page < TOTAL_PAGES; ++page) {
                this.compositeDisposable.add(this.movieAPI.getPopular(APIClient.API_KEY_VALUE, APIClient.LANGUAGE, page).flatMapIterable(movies -> {
                    this.preferences.edit().putBoolean(contextWeakReference.get().getString(R.string.pref_key_first_time), false).apply();
                    ...
                    return movies;
                }).flatMap((Function<MovieModel, Observable<ArrayList<VideoModel>>>) movie -> this.movieAPI.getVideosOf(movie.getId(), APIClient.API_KEY_VALUE, APIClient.LANGUAGE), Pair::new).flatMap((Function<Pair<MovieModel, ArrayList<VideoModel>>, Observable<Credits>>) movieAndVideos -> {
                ...
                }, (movieAndVideos, credits) -> Triplet.of(movieAndVideos.first, movieAndVideos.second, credits)).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(movieAndVideosAndCredits -> {
                    ...
                    if (movie != null) {
                        ...
                        } else movie.setYoutubeKeyVideo("ScMzIvxBSi4");
                        ...
                        //Fetching Actors
                        if (!actors.isEmpty()) movie.setActorsFullPosterPaths(uniqueActors);
                    ...
                    String errorMessage;
                    if (throwable.getMessage() == null) errorMessage = "Unknown Error";
                    else errorMessage = throwable.getMessage();
            ...
            refreshing.setValue(false);
            firstTime.setValue(preferences.getBoolean(contextWeakReference.get().getString(R.string.pref_key_first_time), true));
    ...
    private void setLastUpdateNow() {
        this.preferences.edit().putString(this.contextWeakReference.get().getString(R.string.pref_key_last_update), DateUtils.getCurrentDateTime()).apply();
    }
